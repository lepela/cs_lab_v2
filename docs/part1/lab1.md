...
> **이번 랩 완성물**: 내 환경에 `HR 채용 자동화` 솔루션이 배포되고, 
>
> **목표**: 흐름 2개를 실행해 **내 지원자 리스트 42명**이 준비된
>
...
{: .note }
**면접관 에이전트는 이 솔루션에 없습니다.** Lab 2에서 여러분이 직접 만듭니다. Lab 1은 그 에이전트가 다룰 **데이터(지원자 리스트)** 를 준비하는 단계입니다.
...
3. 상단 명령 모음에서 **가져오기 → 솔루션 가져오기**를 클릭합니다.
    ...
    ![솔루션 가져오기 다이얼로그 → 찾아보기](../../assets/lab1/03b.png)
...
{: .note }
**데이터 채우기는 선택입니다.** 빈 리스트만 있으면 되는 경우 이 흐름을 건너뛰고, **적재 흐름(Lab 4)** 으로 지원자를 직접 넣어도 됩니다. 다만 Lab3에서 조회·평가 실습에는 데이터가 필요하니 오늘은 둘 다 실행 하길 권장합니다.
