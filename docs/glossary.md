...
| <span id="term-c1c2c3">**C1 / C2 / C3 (모듈 1 기능)**</span> | 면접관 에이전트가 **SharePoint 커넥터 도구와 지침**으로 제공하는 세 가지 기능. C1 지원자 조회·C2 적합도 평가는 "항목 가져오기" 커넥터 하나로 구현되고, C3 면접 질문 생성은 원문이 필요할 때 "경로로 파일 콘텐츠 가져오기"가 추가된다. 별도 흐름 없이 커넥터+지침으로 동작하며, 상태 변경이 수반되는 면접 확정은 별도 흐름(Lab 6)이 담당한다 (Lab 3) |
