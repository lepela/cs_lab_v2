...
{: .time }
**30분 타이머.** 이름 기반 조회 → 두 겹 게이트 → 원자 갱신. 흐름이 에이전트 대신 안전을 보장합니다.
...
4. **+ 작업 추가** → SharePoint **항목 가져오기(2개 이상)**를 추가합니다.
    ...
    **사이트 주소** = `SPSiteUrl` 칩, **목록 이름** = 본인 목록으로 지정합니다.
...
14. 면접관 에이전트 **도구**에 이 흐름을 추가합니다. **+ 도구 추가** → **흐름** 탭에서 **AI적합도변경**을 선택합니다.
...
17. 추천된 적합도와 다른 값을 요청 받으면 지침에 의한 가드레일이 동작합니다(확률적).
...
{: .important }
흐름은 **이름**을 받아 조회·게이트·갱신을 자급자족합니다. **Lab 7**에서는 전형단계 변경에 토픽이 왜 필요한지 — 사람 확인(HITL)과 ID 해소를 토픽이 어떻게 맡는지 — 를 다룹니다.
